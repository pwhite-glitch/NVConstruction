...
export async function POST(request) {
  // Derive inviter identity from session — never trust inviter_id from body
  const auth = await requireAuth(request)
  if (auth.error) return auth.error

  try {
    const { company_id, email: rawEmail, full_name, role } = await request.json()
    if (!company_id || !rawEmail) {
      return Response.json({ error: 'company_id and email are required' }, { status: 400 })
    }

    // Fetch inviter profile to check their company and name
    ...
    // PM can invite to any company; sub-admins/sub-pms can only invite to their own company
    const canInvite = isPM(auth.role) ||
      (['sub_admin', 'sub_pm'].includes(auth.role) && inviterProfile?.company_id === company_id)
    if (!canInvite) {
    ...
    const { data: company } = await adminSupabase.from('companies').select('name').eq('id', company_id).single()
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0a;padding:40px 20px;">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#141414;border:1px solid #222;border-radius:16px;overflow:hidden;">
        <tr><td style="border-bottom:1px solid #222;padding:28px 40px;text-align:center;">
        ...
        <tr><td style="padding:40px;">
          ...
          <p style="margin:0 0 24px;font-size:14px;color:#888;line-height:1.6;">${inviterName} has invited you to join <strong style="color:#f1f1f1;">${companyName}</strong> on the NV Construction portal.</p>
          <table cellpadding="0" cellspacing="0" style="margin:0 0 32px;">
            <tr><td style="background:#e8590c;border-radius:8px;">
              <a href="${inviteUrl}" style="display:inline-block;padding:14px 32px;font-size:14px;font-weight:700;color:#fff;text-decoration:none;">Accept Invitation</a>
            </td></tr>
          </table>
        </td></tr>
        <tr><td style="border-top:1px solid #1e1e1e;padding:20px 40px;text-align:center;">
          <p style="margin:0;font-size:12px;color:#444;">Sent by NV Construction.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
