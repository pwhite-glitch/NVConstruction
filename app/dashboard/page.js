...
export default function Dashboard() {
  ...
  async function saveBillingEdit() {
    ...
    if (editBillingFile) {
      ...
      await authFetch('/api/billing-entry', { method: 'PATCH', body: formData })
    } else {
      await authFetch('/api/billing-entry', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(patchData) })
    }
  ...
  async function deleteDirEntry(id) {
    ...
    const res = await authFetch('/api/delete-sub', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ directory_id: id }) })
  ...
  async function getDocUrl(filePath) {
    const res = await authFetch('/api/sub-docs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'signed-url', file_path: filePath }) })
    const data = await res.json()
    if (data?.url) window.open(data.url, '_blank')
  }

  async function downloadDoc(filePath, fileName) {
    const res = await authFetch('/api/sub-docs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'signed-url', file_path: filePath, download: true }) })
    const data = await res.json()
    if (data?.url) {
  ...
  async function uploadSubDoc(dirId, type, file) {
    ...
    const urlRes = await authFetch('/api/sub-docs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'upload-url', path }) })
    ...
    await authFetch('/api/sub-docs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ directory_id: dirId, [field]: path }) })
  ...
  async function fixProfile() {
    ...
    const res = await authFetch('/api/fix-profile', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
<div class="doc-footer">
  ...
  async function loadBD() {
    ...
    if (jobIds.length > 0) {
      const pr = await authFetch('/api/job-profit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ job_ids: jobIds }) })
                  ...
                  <div style={{ ...s.formBox, marginBottom: '1.25rem' }}>
                    ...
                    <form onSubmit={addJob}>
                      ...
                      <div style={{ ...s.grid2, marginBottom: '12px' }} className="rx-grid-2">
                        ...
                        <div>
                          <label style={s.label}>{(newJob.nv_role || 'gc') === 'sub' ? 'GC billing type' : 'Prime billing type'}</label>
